...
WITH
        ...
        SELECT DISTINCT
            ...
            MAX(
                CASE
                    ...
                    ELSE 'No'
                END
            ) AS "PrintChar_PseudoCryptoglyph",
            MAX(
                CASE
                    WHEN z09char.ATINN = '0000028227'
    ...
    tpm_data AS (
        SELECT DISTINCT
            names.PNAME AS TPM,
            names.PNTEXT_UP AS TPMTXT,
            status.STATUS AS TPM_STATUS,
            relations.GLPT,
            relations.GLPTTXT,
            relations.ECLASS,
            relations.ECLASSTXT,
            relations.ECLASS_S,
            relations.ECLASS_STXT,
            -- Include all the TPM characteristics from PNCMP table
            pncmp.TPM_PLACEHOLDER_CHECK,
            pncmp.TPM_PLANT,
            pncmp.TPM_BASE_UNIT,
            pncmp.TPM_VALID_DATE,
            pncmp.TPM_INVALID_DATE,
            pncmp.TPM_PREDECESSOR_MATNUM,
            pncmp.TPM_SUCCESSOR_MATNUM,
            pncmp.TPM_PACKAGING_CHAR,
            pncmp.TPM_PMD_NO,
            pncmp.PNGUID,
            names.CREABY,
            names.CREADAT,
        ...
        FROM
            ...
            INNER JOIN tpm_node_names names ON pncmp.PNGUID = names.PNGUID
            INNER JOIN tpm_node_status status ON pncmp.PNGUID = status.PNGUID
            INNER JOIN tpm_node_relations relations ON pncmp.PNGUID = relations.TPM_PNGUID
        WHERE
            names.PNTYPE = 'Z_TPM'
    ),
...
SELECT DISTINCT
    ...
    RIGHT (MATDATA.MATNR, 8) AS "MATNR8",
