...
class TestAsyncEndpoints:
    ...
    async def test_async_get_swatch_config_not_found(self):
        """Test get_swatch_config endpoint with invalid colorname using async client."""
        async with AsyncClient(app=app, base_url="http://test") as ac:
...
class TestIntegrationScenarios:
    ...
    async def test_complete_workflow_scenario(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            ...
            if all_swatches:
                first_colorname = all_swatches[0]["colorname"]
                specific_response = await ac.get(f"/get_swatch_config?colorname={first_colorname}")
                assert specific_response.status_code == 200
                
                specific_swatch = specific_response.json()["swatches"][0]
                assert specific_swatch["colorname"] == first_colorname
    
    def test_concurrent_requests_sync(self, client):
        """Test that multiple concurrent synchronous requests work correctly."""
        import concurrent.futures
        
        def make_request(colorname):
            return client.get(f"/get_swatch_config?colorname={colorname}")
        
        # Test concurrent requests for different colornames
        colornames = ["DIELINE", "PA123", "PA321"]
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(make_request, name) for name in colornames]
