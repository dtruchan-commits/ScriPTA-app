...
class TestGetLayerConfigEndpoint:
    ...
    def test_get_layer_config_with_valid_config_name_default(self, client):
        ...
        expected_layer_names = ["DIELINE", "TECHNICAL", "BRAILLE_EMB", "TEXT", "ACF_HRL", "ACF_LRA_VARNISH", "DESIGN", "INFOBOX", "GUIDES"]
    ...
    def test_layer_config_response_schema_compliance(self, client):
        ...
        for config in data:
            ...
            for layer in config["layers"]:
                ...
                # Validate enum values
                valid_layer_names = ["DIELINE", "TECHNICAL", "BRAILLE_EMB", "TEXT", "ACF_HRL", "ACF_LRA_VARNISH", "DESIGN", "INFOBOX", "GUIDES", "PANEL"]
                valid_colors = ["GOLD", "TEAL", "FIESTA", "LIGHT_BLUE", "YELLOW", "GREEN", "RED", "LAVENDER", "GRAY", "BLUE"]
    ...
    def test_layer_config_specific_layer_properties(self, client):
        ...
        assert dieline_layer["locked"] is True
        assert dieline_layer["print"] is True
        assert dieline_layer["color"] == "GOLD"

        guides_layer = next(layer for layer in config["layers"] if layer["name"] == "GUIDES")
        assert guides_layer["locked"] is True
        assert guides_layer["print"] is False
        assert guides_layer["color"] == "GRAY"

        text_layer = next(layer for layer in config["layers"] if layer["name"] == "TEXT")
        assert text_layer["locked"] is False
        assert text_layer["print"] is True
